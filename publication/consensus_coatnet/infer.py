...
def run(root,checkpoint,out,device='cuda',series_csv='test_series.csv',
        series_directory='test_series',sample_csv='sample_submission.csv'):
    ...
    if set(sample.columns)!=set([UID]+TARGETS) or sample[UID].duplicated().any() or not len(sample):
        raise ValueError('Unexpected sample schema/UIDs')
    if not set(sample[UID])<=set(series[UID]): raise ValueError('Test metadata missing sample study')
    groups={uid:frame for uid,frame in series.groupby(UID,sort=False)}
    audit=Counter(); times=[]; predictions=[]; start=time.monotonic()
    for i,uid in enumerate(sample[UID]):
        ...
        if (i+1)%25==0: print(f'Predicted {i+1}/{len(sample)} studies',flush=True)
    pred=np.stack(predictions)
    if pred.shape!=(len(sample),12) or not np.isfinite(pred).all() or ((pred<0)|(pred>1)).any():
        raise ValueError('Invalid predictions')
    sample[TARGETS]=pred; sample.to_csv(out,index=False)
    save_json(out.with_suffix('.audit.json'),dict(n_studies=len(sample),elapsed_seconds=time.monotonic()-start,
