"""Day-2 remaining open-code pipeline: infer all, then submit.

Pushes at most the remaining notebooks when a T4 slot is free.
Does not submit until inference artifacts exist, so scoring GPU jobs
do not block later inference. Submits each completed version once.
"""
...
STATE_PATH = ROOT / "tmp" / "day2_state.json"
LOG_PATH = ROOT / "tmp" / "day2_orchestrator.log"
...
JOBS = [
    {
        "name": "evgen",
        "folder": str(ROOT / "tmp" / "ready_d2" / "d2-evgen"),
        "kernel": "dmitriigluzdov/rsna-open-sol-v6",
        "message": "open fork evgendvorkin/rsna-baseline public 0.936",
        "backup_folder": str(ROOT / "tmp" / "ready_d2" / "d2-ayodeji"),
        "backup_message": (
            "open fork ayodejiibrahimlateef/rsna-knee-c01-dinosaur-v4-dual-fusion public 0.936"
        ),
    },
    {
        "name": "shashwat",
        "folder": str(ROOT / "tmp" / "ready_d2" / "d2-shashwat"),
        "kernel": "dmitriigluzdov/rsna-open-sol-fusion",
        "message": "open fork shashwat1729/rsna-knee-infer-gpu-hsv44fixc-calw public 0.936",
    },
    {
        "name": "nishant",
        "folder": str(ROOT / "tmp" / "ready_d2" / "d2-nishant"),
        "kernel": "dmitriigluzdov/rsna-open-sol-raptor",
        "message": "open fork nishantkharga/bend-the-knee-to-dinov3-ensembled-nk public 0.935",
    },
]
...
def maybe_push(api: KaggleApi, state: dict) -> None:
    ...
    for job in JOBS:
        st = state["jobs"][job["name"]]
        if st["pushed"] and not st["has_csv"] and st.get("error") != "permanent":
            status = st.get("status") or ""
            if any(x in status for x in ("RUNNING", "QUEUED", "PENDING")):
                running_like += 1
    # Still try the next unpushed job even if we cannot know other users' GPU
    # occupancy; Kaggle will reject with the session cap.
    for job in JOBS:
        st = state["jobs"][job["name"]]
...
def refresh_statuses(api: KaggleApi, state: dict) -> None:
    for job in JOBS:
        st = state["jobs"][job["name"]]
...
def main() -> int:
    ...
    while time.time() < deadline:
        refresh_statuses(api, state)
        if done(state):
            log("all remaining jobs submitted or permanent")
            return 0
        # Infer first: only submit when every job is terminal.
