"""Validate visible standalone S24 3D kernel output before submit."""
...
def main(output_dir: Path) -> dict:
    ...
    receipt_path = output_dir / "s24_local_receipt.json"
    ...
    if receipt_path.is_file():
        ...
        if receipt.get("status") != "S24_STANDALONE_PASS":
            raise RuntimeError(f"status {receipt.get('status')}")
        if receipt.get("blend") != "none_local_3d_only":
    ...
    result = {
        ...
        "hidden_estimate_seconds": None if receipt.get("seconds") is None else round(float(receipt["seconds"]) * (1322 / max(len(uids), 1)), 1),
    }
