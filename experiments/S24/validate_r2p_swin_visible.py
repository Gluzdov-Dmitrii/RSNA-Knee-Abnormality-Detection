"""Validate visible r2plus1d+Swin rank-mean kernel output before submit."""
...
def main(output_dir: Path) -> dict:
    ...
    receipt_path = output_dir / "s24_r2p_swin_receipt.json"
    ...
    if len(uids) >= 32:
        ...
        varying = sum(int(pd.unique(frame[label]).size >= 2) for label in HEADER[1:])
        if varying == 0:
            raise RuntimeError("all prediction columns are constant")
    receipt = {}
    if receipt_path.is_file():
        receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
        if receipt.get("status") != "S24_R2P_SWIN_PASS":
            raise RuntimeError(f"status {receipt.get('status')}")
        if receipt.get("blend") != "rank_mean_r2p_swin":
            raise RuntimeError("unexpected blend")
        if receipt.get("members") != ["r2plus1d_18", "swin3d_t"]:
