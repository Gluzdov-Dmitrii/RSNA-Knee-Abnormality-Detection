# S24_STANDALONE_BAG
# PIXEL_CACHE_V1 live decode + 5-fold 3D bag. No A0/Raptor mix.
# VARIANT is patched by build_kernel.py: r3d_18 | r2plus1d_18
...
DATASET_SLUG = {
    "r3d_18": "rsna-s24-r3d18-9slice-folds",
    "r2plus1d_18": "rsna-s24-r2plus1d-9slice-folds",
}[VARIANT]
WEIGHT_ROOTS = [
    Path(f"/kaggle/input/{DATASET_SLUG}"),
    Path(f"/kaggle/input/datasets/dmitriigluzdov/{DATASET_SLUG}"),
]
T0 = time.time()


def log(msg: str) -> None:
    print(f"[S24 {VARIANT} {time.time() - T0:.0f}s] {msg}", flush=True)
...
def weight_root() -> Path:
    for path in WEIGHT_ROOTS:
        if (path / "manifest.json").is_file() and (path / "fold0.pt").is_file():
            return path
    raise FileNotFoundError(f"{DATASET_SLUG} is not mounted")
...
def make_trunk():
    if VARIANT == "r3d_18":
        from torchvision.models.video import r3d_18 as ctor
...
class S24Video3D(nn.Module):
    def __init__(self, n_targets: int = 12, dropout: float = 0.25):
        super().__init__()
...
def load_folds(root: Path, device: torch.device) -> list[nn.Module]:
    ...
    if manifest.get("n_targets") != 12:
        raise RuntimeError("S24 manifest n_targets drift")
    if manifest.get("layout") != "vol9":
        raise RuntimeError("S24 manifest layout drift")
    expected_backbone = {
        "r3d_18": "torchvision.r3d_18",
        "r2plus1d_18": "torchvision.r2plus1d_18",
    }[VARIANT]
...
bag = np.full((len(uids), 12), 0.5, dtype=np.float64)
...
with torch.no_grad():
    for start in range(0, len(uids), chunk):
        ...
        bag[start : start + len(decoded)] = np.mean(probs, axis=0)
        log(f"decoded {start + len(decoded)}/{len(uids)}")

if int(present.sum()) == 0:
    raise RuntimeError("S24 decoded zero studies with a present slot")

out = sample.copy()
for col, label in enumerate(LABELS):
    ranked = rank_pct(bag[:, col])
...
receipt = {
    "schema_version": "s24_standalone_3d_bag_v1",
    "status": "S24_STANDALONE_PASS",
    "experiment_id": "S24",
    "variant": VARIANT,
    "n_studies": len(uids),
    ...
    "blend": "none_local_3d_only",
}
(WORK / "s24_local_receipt.json").write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
log("S24_STANDALONE_PASS " + receipt["submission_sha256"][:12] + f" seconds={receipt['seconds']}")
