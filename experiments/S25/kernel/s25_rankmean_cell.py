# S25_RANKMEAN_R3D_R2P
# PIXEL_CACHE_V1 live decode once + 5-fold r3d_18 and r2plus1d_18 bags.
...
MEMBERS = (
    {
        "variant": "r3d_18",
        "slug": "rsna-s24-r3d18-9slice-folds",
...
def log(msg: str) -> None:
    print(f"[S25 rankmean {time.time() - T0:.0f}s] {msg}", flush=True)
...
class S24Video3D(nn.Module):
    def __init__(self, variant: str, n_targets: int = 12, dropout: float = 0.25):
        super().__init__()
...
def load_folds(root: Path, device: torch.device, variant: str, expected_backbone: str) -> list[nn.Module]:
    ...
    for fold in range(5):
        ...
        model = S24Video3D(variant).to(device)
...
for member in MEMBERS:
    ...
    models = load_folds(root, device, member["variant"], member["backbone"])
...
if int(present.sum()) == 0:
    raise RuntimeError("S25 decoded zero studies with a present slot")
...
for col, label in enumerate(LABELS):
    ...
    if len(uids) >= 3 and np.unique(ranked).size < 2:
        raise RuntimeError(f"{label} rank-mean is constant")
    out[label] = ranked
...
receipt = {
    "schema_version": "s25_rankmean_r3d_r2p_v1",
    "status": "S25_RANKMEAN_PASS",
    "experiment_id": "S25",
    ...
    "blend": "rank_mean_r3d_r2p",
}
(WORK / "s25_local_receipt.json").write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
log("S25_RANKMEAN_PASS " + receipt["submission_sha256"][:12] + f" seconds={receipt['seconds']}")
