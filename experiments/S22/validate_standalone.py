"""Validate visible standalone S22 CNN kernel output before submit."""
...
def main(output_dir: Path) -> dict:
    ...
    receipt_path = output_dir / "s22_local_receipt.json"
    ...
    if receipt_path.is_file():
        ...
        if receipt.get("status") != "S22_LOCAL_CNN_PASS":
            raise RuntimeError(f"status {receipt.get('status')}")
        if receipt.get("blend") != "none_local_cnn_only":
