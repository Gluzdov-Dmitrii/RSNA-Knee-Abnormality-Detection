# S22_A0_TRANSFORMER_MIX_OVERLAY
# PIXEL_CACHE_V1 live decode + 5-fold ResNet-18 bag, then replace 40% of the
# A0 transformer rank component. Medial Meniscus keeps T30/R60/bag10 on the mixed transformer.
...
def bag_meniscus_rank(uids: list[str]) -> np.ndarray | None:
    path = WORK / "public0033_bag_raw.csv"
    if not path.is_file():
        return None
    frame = pd.read_csv(path, dtype={"StudyInstanceUID": str})
    if "Medial Meniscus" not in frame.columns:
        return None
    by_uid = dict(zip(frame["StudyInstanceUID"].astype(str), frame["Medial Meniscus"].to_numpy(np.float64)))
    if any(uid not in by_uid for uid in uids):
        return None
    return rank_pct(np.asarray([by_uid[uid] for uid in uids], dtype=np.float64))
...
a0_path = WORK / "submission.csv"
if not a0_path.is_file():
    raise FileNotFoundError(a0_path)
a0_sha = sha256_file(a0_path)
a0 = pd.read_csv(a0_path, dtype={"StudyInstanceUID": str})
if a0["StudyInstanceUID"].astype(str).tolist() != uids:
    raise RuntimeError("A0 UID order drift vs sample_submission.csv")
for name in ("_blend_tr", "_blend_cr", "_blend_transformer", "_blend_coatnet"):
    if name not in globals():
        raise RuntimeError(f"A0 rank component missing: {name}")
if _blend_transformer["StudyInstanceUID"].astype(str).tolist() != uids:
    raise RuntimeError("transformer UID order drift")
if _blend_coatnet["StudyInstanceUID"].astype(str).tolist() != uids:
    raise RuntimeError("raptor UID order drift")
...
s22 = np.zeros((len(uids), 12), dtype=np.float64)
...
bag_rank = bag_meniscus_rank(uids)
out = a0.copy()
changed = {}
for col, label in enumerate(LABELS):
    s22_rank = rank_pct(s22[:, col])
    transformer = np.asarray(_blend_tr[label].to_numpy(), dtype=np.float64)
    raptor = np.asarray(_blend_cr[label].to_numpy(), dtype=np.float64)
    if transformer.shape != (len(uids),) or raptor.shape != (len(uids),):
        raise RuntimeError(f"component shape drift: {label}")
    tr_mix = (1.0 - REPLACEMENT) * transformer + REPLACEMENT * s22_rank
    if label == "Medial Meniscus" and bag_rank is not None:
        mixed = MENISCUS_W[0] * tr_mix + MENISCUS_W[1] * raptor + MENISCUS_W[2] * bag_rank
    else:
        mixed = (1.0 - DEFAULT_RAPTOR_W) * tr_mix + DEFAULT_RAPTOR_W * raptor
    ranked = rank_pct(mixed)
    if not np.isfinite(ranked).all() or ranked.min() < 0 or ranked.max() > 1:
        raise RuntimeError(f"invalid overlay rank: {label}")
    if len(uids) >= 3 and np.unique(ranked).size < 2:
        raise RuntimeError(f"{label} overlay is constant")
    before = a0[label].to_numpy(np.float64)
    changed[label] = int(np.sum(np.abs(ranked - before) > 1e-12))
    out[label] = ranked

out_path = WORK / "submission_S22.csv"
tmp_path = WORK / ".submission_S22.tmp.csv"
out.to_csv(tmp_path, index=False)
tmp_path.replace(out_path)
WORK.joinpath("submission.csv").write_bytes(out_path.read_bytes())
...
receipt = {
    "schema_version": "s22_a0_transformer_mix_v1",
    "status": "S22_OVERLAY_PASS",
    "experiment_id": "S22",
    "replacement_share": REPLACEMENT,
    "default_raptor_outer_weight": DEFAULT_RAPTOR_W,
    "n_studies": len(uids),
    ...
    "parent_a0_sha256": a0_sha,
    "changed_rows": changed,
    "bag_meniscus": bag_rank is not None,
    "seconds": round(time.time() - T0, 1),
    "internet_expected_off": True,
}
(WORK / "s22_overlay_receipt.json").write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
log("S22_OVERLAY_PASS " + receipt["submission_sha256"][:12] + f" seconds={receipt['seconds']}")
