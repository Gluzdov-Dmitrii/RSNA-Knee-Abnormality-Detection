"""Strip S24 r3d_18 fold checkpoints to fp16 state_dicts.
...
DEFAULT_SRC = PROJECT / "runs/20260910T0545Z-s24-r3d18-9slice/checkpoints"
DEFAULT_OUT = PROJECT / "export/s24-r3d18-9slice-folds"
CODE_CANDIDATES = [
...
def main() -> None:
    ...
    for code in CODE_CANDIDATES:
        if (code / "s24_r3d18_9slice.py").is_file() and str(code) not in sys.path:
            sys.path.insert(0, str(code))
            break
    from s24_r3d18_9slice import build_model
    ...
    for fold in range(5):
        ...
        model.eval()
        with torch.no_grad():
            dummy_x = torch.zeros(2, 3, 3, 9, 224, 224)
            dummy_mask = torch.ones(2, 3)
    ...
    manifest = {
        "schema_version": "s24_r3d18_9slice_folds_fp16_v1",
        "experiment_id": "S24",
