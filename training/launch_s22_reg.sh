...
RUN=$PROJECT/runs/20260910T0335Z-s22-resnet18-reg
...
export TMPDIR=/tmp/rsna-s22-reg
...
nohup "$PY" -u "$CODE/s22_resnet18_25d.py" --train \
  --epochs 24 --batch-size 16 --lr 1e-4 --workers 4 --patience 8 \
  --dropout 0.4 --weight-decay 1e-3 --mixup 0.4 --backbone-lr-mult 0.1 \
  --experiment-key S22_RESNET18_25D_REG \
